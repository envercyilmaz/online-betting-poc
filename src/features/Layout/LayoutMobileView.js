...
const LayoutMobileView = ({
  ...
  // Using react-virtualized for list rendering performance
  const renderRow = ({ list, index, style }) => {
    const item = list[index];
    return (
      <div className='betting-section-mobile' key={item.C} style={style}>
            ...
            <div onClick={handleBettingAreaClick}>
               ...
               <AutoSizer>
                {({ width, height }) => <List
                  width={width}
                  height={height}
                  rowRenderer={({ style, index }) => renderRow({ list, index, style })}
